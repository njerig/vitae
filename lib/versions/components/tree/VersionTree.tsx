...
const BRANCH_COLORS = ["var(--accent)", "#6366f1", "#059669", "#d97706"]
...
export function VersionTree({
  ...
  if (versions.length === 0) {
    return (
      <p style={{ color: "var(--ink-fade)", fontSize: "0.9rem" }}>No versions in this group.</p>
    )
  }
          ...
          {nodes.map((node) => {
            ...
            return (
              <g
                ...
                onClick={() =>
                  setSelectedId(selectedId === node.version.id ? null : node.version.id)
                }
              >
